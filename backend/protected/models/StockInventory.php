...
class StockInventory extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, date, company_id', 'required'),
			array('create_uid, write_uid, company_id', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>64),
			array('create_date, write_date, state, date_done', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, create_uid, create_date, write_date, write_uid, state, name, date_done, date, company_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'stockInventoryLines' => array(self::HAS_MANY, 'StockInventoryLine', 'inventory_id'),
			'stockInventoryMoveRels' => array(self::HAS_MANY, 'StockInventoryMoveRel', 'inventory_id'),
		);
	}
