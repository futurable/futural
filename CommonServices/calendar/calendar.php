...
calendar = {
	month_names: ["<?php echo gettext('January'); ?>"
	...
	selectDate:function(year,month,day) {
		...
		document.getElementById(ths.opt["input"]).value = day + "." + month + "." + year;//year + "-" + month + "-" + day; // Date format is :HARDCODE:
		ths.hideCalendar();
	},
	...
	makeCalendar:function(year, month, day) {
		...
		for(var i=0;i<=5;i++) {
			...
			for(var j=0;j<7;j++) {
				...
				else if(j >= start_day -1 && !flag) flag=1;//If the first day of this month has come, start the date writing
															// start_day -1 because we want that Monday is weeks first day (not Sunday)
		...
		var cbutton = "<?php print gettext('Cancel');?>";
		this.wrt("</table>");
		this.wrt("<input type='button' value='"+cbutton+"' class='calendar-cancel' onclick='calendar.hideCalendar();' />");
