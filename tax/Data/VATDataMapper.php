 ...
 * @version   2011-09-07
 */
class VATDataMapper extends DataMapper {
	...
	protected function doSave( $object ) {
		...
		if ( $object instanceof VAT ) {
			Debug::debug(get_class(), "doSave", "Parameter object is instance of VAT", 2);
	...
	protected function doUpdate( $object ) {
		...
		// check object
		if ( $object instanceof VAT ) {
			Debug::debug(get_class(), "doUpdate", "Parameter object is instance of VAT", 2);
			print "doUpdate VAT";
		}
	...
	protected function doDelete( $object ) {
		...
		// check object
		if ( $object instanceof VAT ) {
			Debug::debug(get_class(), "doDelete", "Parameter object is instance of VAT", 2);
			// delete object
			print "doDelete VAT";
		}
	...
	/**
	 * Function to fill VAT declaration objects in spesific time range
		 ...
		 and ( DataValidator::isDateISOSyntaxValid( $endDate ) or $endDate == false ) ) {
			...
			if ($result) {
				...
				while ($row = mysql_fetch_assoc($result)){
					...
					$VAT->fillObjectFromArray($VAT, $row);
					$taxAccountObject->setOneTransactionToTransactions( $VAT );
					unset( $VAT );
				}
			}
		}
